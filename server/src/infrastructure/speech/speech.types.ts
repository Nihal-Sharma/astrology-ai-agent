...
/**
 * Provider-independent STT interface — mirrors `LlmClient`
 * (infrastructure/llm/llm.types.ts). Callers (RealtimeService)
 * depend on this, never on a concrete provider class, so the
 * provider can be swapped via config/container wiring alone.
 */
...
/** Provider-independent TTS interface — see STTClient's doc comment. */
