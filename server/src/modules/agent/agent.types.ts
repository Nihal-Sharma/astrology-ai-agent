...
/**
 * Gold-tier voice turn (ROADMAP.md's Phase C) — `AgentTurnInput`
 * with `message` replaced by raw `audio`: there's no transcript
 * yet, the planner call derives one as part of its own routing
 * decision (see AgentOrchestrator.streamTurnWithAudio /
 * PlannerService.createPlan). Always a voice turn by definition,
 * so no `inputType` field.
 */
export interface AgentAudioTurnInput {
  userId: string;
